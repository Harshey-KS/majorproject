...
//       {cards?.map((card) => (
//         <div key={card?._id} className="bg-blue-100 rounded-xl w-52 h-64 overflow-hidden bigShadow flex flex-col justify-between">
//           <div>
//             <div
...
//                 }))
//               }
//               className="flex items-center justify-center bg-[#5932EA] h-10 text-white font-medium p-2 shadow-md shadow-indigo-500/40" autoFocus>
//               {card?.className}
//             </div>
//             <ul className="text-[#5C5F65] mt-2 w-full px-4">
//               {editable?._id === card?._id
//                 ? (editable?.portionCovered || []).map((p, i) => (
//                     <li key={i} className="flex m-1 h-4 items-center w-full">
...
//                     <li key={p} className="flex m-1 items-center w-full">
//                       <BsFillCircleFill className="h-2 w-2 mr-3" />
...
const Cards = ({ cards = [], updatePortion, setTeacher }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const [newClass, setNewClass] = useState({ className: "", portion: [] });
  const [editable, setEditable] = useState({ className: "", portionCovered: [] })

  console.log(editable);
  async function handleDoneClick() {
    try {
      await updatePortion([...cards, editable]);
      setEditable(null);
    } catch (error) {
...
// const Cards = ({ cards, updatePortion, setTeacher }) => {
//   const [modalOpen, setModalOpen] = useState(false);
//   const [newClass, setNewClass] = useState({ className: "", portionCovered: [] });
//   const [editable, setEditable] = useState(null);
...
//     <div className="grid mt-14 grid-cols-3 gap-4">
//       {cards?.map((card) => (
//         <div key={card._id} className="bg-white rounded-xl w-52 h-64 overflow-hidden bigShadow flex flex-col justify-between">
//           <div>
//             <div
//               contentEditable={card._id === editable?._id}
//               onInput={(e) =>
...
//                     </li>
//                   ))
//                 : card.portionCovered?.map((p, i) => (
//                     <li key={i} className="flex m-1 items-center w-full">
//                       <BsFillCircleFill className="h-2 w-2 mr-3" />
//                       <p className="break-all w-5/6 text-left">{p}</p>
//                     </li>
//                   ))}
...
//                 onClick={async () => {
//                   await updatePortion([
//                     ...cards?.map((e) => (e._id === editable?._id ? editable : e)),
//                   ]);
//                   setEditable(null);
//                 }}
//               >
...
//           <button
//             onClick={() => {
//               setTeacher((prev) => ({ ...prev, classes: [...prev.classes, newClass] }));
//               updatePortion([...cards, newClass]);
//               setModalOpen(false);
//             }}
//           >
